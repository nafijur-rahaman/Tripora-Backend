...
app.post("/api/book_package/", verifyToken, async (req, res) => {
  try {
    ...
    if (!package_id || !ObjectId.isValid(package_id)) return res.status(400).send({ success: false, message: "Valid package ID is required" });
...
// Delete booking and decrement package booking count
app.delete("/api/delete_booking/", verifyToken, async (req, res) => {
  const { booking_id, package_id } = req.body;
  if (!booking_id || !ObjectId.isValid(booking_id)) return res.status(400).send({ success: false, message: "Valid booking ID is required" });
  if (!package_id || !ObjectId.isValid(package_id)) return res.status(400).send({ success: false, message: "Valid package ID is required" });

  try {
    const deleteResult = await packageBookingsCollection.deleteOne({ _id: new ObjectId(booking_id) });
    if (deleteResult.deletedCount === 0) return res.status(404).send({ success: false, message: "Booking not found" });
...
app.get("/api/get_all_bookings", verifyToken, async (req, res) => {
  try {
    const userEmail = req.query.userEmail;
    if (!userEmail) return res.status(400).send({ success: false, message: "User email is required" });
